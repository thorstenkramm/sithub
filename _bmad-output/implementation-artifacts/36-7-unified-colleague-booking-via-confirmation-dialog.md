...
- [ ] Task 1: Create a reusable colleague-select fragment for booking dialogs (AC: #1, #2, #3, #5)
  - [ ] Extract the colleague radio + autocomplete pattern that already lives in
  ...
  - [ ] Reuse i18n keys already present: `items.bookForMyself`, `items.bookForColleague`,
  ...
  - [ ] Reuse `fetchColleagues()` (`web/src/api/users.ts:20-22`) and `BookOnBehalfOptions`
        (`web/src/api/bookings.ts:55-58`). Colleague name resolution follows the existing
        `resolveColleagueName` pattern.

- [ ] Task 2: Tile view — replace the inline dropdown with a shared confirmation dialog (AC: #1, #2, #5)
  - [ ] Remove the top-level inline colleague `v-autocomplete`
        (`web/src/views/ItemsView.vue:122-136`, `data-cy="colleague-select"`) and its inline CSS
        (`:2225-2233`, `.colleague-select-inline`).
  - [ ] Add a booking confirmation dialog to `ItemsView.vue` that contains the colleague-select
  ...
  - [ ] Rewire the day-mode "Book" button (`ItemsView.vue:353-364`, `data-cy="book-item-btn"`,
  ...
  - [ ] In `bookItem` (`ItemsView.vue:1831-1865`), read the selected colleague from the dialog's
  ...
  - [ ] WEEK MODE: the same top-level `selectedColleagueId` also feeds `submitWeekBookings`
...
- [ ] Task 3: Floor-plan dialog — add colleague selection across all selected days (AC: #3, #4, #5)
  - [ ] Add the colleague-select fragment (Task 1) into the floor-plan booking dialog
  ...
  - [ ] Load colleagues when the dialog opens (`requestBooking` :1460-1473) using
        `fetchColleagues()`; reuse the resolve-name helper pattern.
  - [ ] Thread the on-behalf option into BOTH booking calls in `executeBooking`
        ...
        behalf.
  - [ ] Reset the floor-plan colleague selection when the dialog closes / after a successful
        booking (alongside `pendingBooking.value = null` at :1590-1591), so the next booking
        defaults to "for me" (AC #5).

- [ ] Task 4: Tests (AC: #1-#5)
  - [ ] Vitest component tests for `ItemsView`: the inline `colleague-select` autocomplete is gone;
  ...
  - [ ] Vitest component tests for `InteractiveFloorPlan`: the dialog renders the colleague select;
  ...
  - [ ] Cypress E2E (per AC coverage rule): from the tile view, book a desk on a colleague's behalf
